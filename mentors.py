...
class Lecturer(Mentor):
    ...
    def __str__(self):
        res = f'Имя: {self.name}\nФамилия: {self.surname}\nСредняя оценка за лекции: {self.get_awg_grade(self.grades)}'
        return res
...
some_reviewer = Reviewer('Мистер','Проверкин')
some_reviewer.courses_attached += ['Python']
some_reviewer.courses_attached += ['Git']

next_reviewer = Reviewer('Что-то','С чем-то')
...
next_lecturer = Lecturer('Ваше','Воробейшество')
next_lecturer.courses_attached += ['Python']
next_lecturer.courses_attached += ['Git']



some_student = Student('Максим','Протасов','м')
...
girl_student = Student('Александра','Иванова','ж')
...
print(f'Средняя оценка Александры больше чем у Максима : {"ДА" if girl_student > some_student else "НЕТ"}')
print()

print(f'Средняя оценка Олега Булыгина больше чем у Вашего Воробейшества : {"ДА" if some_lecturer > next_lecturer else "НЕТ"}')
print()
